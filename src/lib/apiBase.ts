...
export default async function apiFetch(endpoint: string, options: RequestInit = {}): Promise<any> {
  ...
  const isAuthRequest = endpoint.includes('/auth/login') || endpoint.includes('/auth/refresh-token');
  const headers = {
